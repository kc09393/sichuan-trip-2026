const itineraries = [
  ...
  { date:"9/6", weekday:"日", title:"九寨溝完整一天", city:"九寨溝", lodging:"Yuji 嶼季海韻酒店", tone:"star", note:"本次旅行第一個重點日。兩位成人票與一位 60 歲以上長者票均已購買，入園時段 08:00–10:00；從飯店步行約 10–15 分鐘到入口。", items:[
    ["07:30","飯店早餐後步行出發","三人早餐已包含；帶各自購票使用的有效證件原件、穿分層衣物，約 07:45 抵達景區入口。","Yuji 嶼季海韻酒店"],
    ["08:00","全天遊覽九寨溝","搭觀光車分段遊覽日則溝、則查洼溝與樹正溝；不追求每個點都走到。","五花海"],
    ["18:00 前","離開景區回飯店","景區 18:00 閉園。若爸媽走累就叫車回 Yuji；晚餐後整理行李，隔天不再排景點。","九寨溝景區入口"]
  ]},
...
const dayGuides = [
  ...
  {effort:"中高",summary:"07:00 飯店早餐 → 07:30 步行出發 → 08:00 入園 → 18:00 前出園。不要自行規劃死板順序。",facts:[
...
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=38").catch(()=>{}));
